...
impl EmbeddedServer {
    ...
    pub fn start(config: EmbeddedConfig) -> FalkorResult<Self> {
        ...
        let process = command
            .spawn()
            .map_err(|e| FalkorDBError::EmbeddedServerError(format!("Failed to start redis-server: {}", e)))?;
    ...
    fn find_redis_server(config: &EmbeddedConfig) -> FalkorResult<PathBuf> {
        if let Some(ref path) = config.redis_server_path {
            ...
            return Err(FalkorDBError::EmbeddedServerError(
                format!("redis-server not found at: {}", path.display())
            ));
        }
    ...
    fn find_falkordb_module(config: &EmbeddedConfig) -> FalkorResult<PathBuf> {
        if let Some(ref path) = config.falkordb_module_path {
            ...
            return Err(FalkorDBError::EmbeddedServerError(
                format!("FalkorDB module not found at: {}", path.display())
            ));
        }
        ...
        if let Some(ref dir) = config.db_dir {
            if !dir.exists() {
                fs::create_dir_all(dir)
                    .map_err(|e| FalkorDBError::EmbeddedServerError(format!("Failed to create db directory: {}", e)))?;
            }
            Ok((dir.clone(), None))
        } else {
            ...
            if !temp_dir.exists() {
                fs::create_dir_all(&temp_dir)
                    .map_err(|e| FalkorDBError::EmbeddedServerError(format!("Failed to create temp directory: {}", e)))?;
            }
    ...
    fn setup_socket_path(config: &EmbeddedConfig, temp_dir: Option<&Path>) -> FalkorResult<PathBuf> {
    ...
    fn create_config_file(db_dir: &Path, socket_path: &Path, db_filename: &str) -> FalkorResult<PathBuf> {
...
"#,
        ...
        fs::write(&config_path, config_content)
            .map_err(|e| FalkorDBError::EmbeddedServerError(format!("Failed to write config file: {}", e)))?;
