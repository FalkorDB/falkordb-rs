...
- An `llms.txt` at the repository root — a curated, machine-readable summary of the public API,
  feature flags, idioms and pitfalls for AI coding assistants (the `llmstxt.org` convention). The
  narrative lives in `docs/llms.template.md`; the `## Public API` section is generated from
  `src/lib.rs`. Run `just llms` to regenerate it, and a `check-llms` CI gate keeps it from drifting
  as the public API changes.
